...
@section('thead_table')
    ...
    <th>{{ trans('mantActivos.th_available')        }}</th>
            ...
            <tr>
                ...
                <td>
                    <select class="form-control" name="available" id="available">
                        <option  value="">{{ trans('mantActivos.isd_available') }}</option>
                        <option value="{{ '0' }}">{{ trans('mantActivos.val_disponible')}}</option>
                        <option value="{{ '1' }}">{{ trans('mantActivos.val_no_disponible') }}</option>
                    </select>
                </td>
        ...
        @foreach($activos as $activo)
            ...
            <tr>
                ...
                @if( $activo->available == 0 )
                    <td>{{ trans('mantActivos.val_disponible')}}</td>
                @else
                    <td>{{ trans('mantActivos.val_no_disponible')}}</td>
                @endif
